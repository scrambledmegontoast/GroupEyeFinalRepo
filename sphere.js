...
//Number of points on the sphere
  ...
  for (let i =  0; i < numPoints; i++) {
    for (let j =  0; j < numPoints; j++) {
      ...
      let x = (radius * sin(theta) * cos(phi)) + random(-dFactor * displacement, +dFactor * displacement)
      let y = (radius * sin(theta) * sin(phi)) + random(-dFactor * displacement, +dFactor * displacement)
      let z = (radius * cos(theta)) + random(-dFactor * displacement, +dFactor * displacement)
